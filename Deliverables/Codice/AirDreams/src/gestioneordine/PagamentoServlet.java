...
public class PagamentoServlet extends HttpServlet {
	...
	private String expDataScadenza = "^\\s*(1[012]|0[1-9])\\/\\d{2}\\s*$";
		...
		try {
			...
			if(numeroCarta!=null) {
				...
				if(!valida(nCarta, titolare, dataScadenza, cvc)) {
					...
					if(!cartaScaduta) {
						if(cartaDiCreditoManager.cercaCarta(nCarta, account.getEmail())!=null) {
			...
			if(carta!=null) {
				...
				for(Biglietto biglietto: biglietti) {
					...
					System.out.println("Bagaglio a mano di biglietto "+biglietto.getCodBiglietto()+" per il volo " + ""+biglietto.getVolo().getId());
